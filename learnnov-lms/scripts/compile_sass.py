        ...
        for dirpath, _, filenames in os.walk(str(source_root)):
            for filename in filenames:
                ...
                if not dry:
                    ...
                    # Convert legacy libsass constants to formats expected by sass.compile
                    output_style_str = {0: "nested", 1: "expanded", 2: "compact", 3: "compressed"}[output_style]
                    source_comments_bool = bool(source_comments)
                    include_paths_str = [str(p) for p in includes]
                    compiled_css = sass.compile(
                        filename=str(source),
                        output_style=output_style_str,
                        source_comments=source_comments_bool,
                    ...
                    output_text = compiled_css
                    # Since we used sass.compile, compilation succeeded if no exception.
