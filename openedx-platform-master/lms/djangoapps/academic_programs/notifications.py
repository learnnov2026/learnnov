...
def send_application_status_notification(application):
    ...
    # محتوى البريد (HTML - يمكن تحسينه لاحقاً بقالب منفصل)
    html_content = f"""
    ...
    try:
        msg = EmailMultiAlternatives(subject, text_content, settings.DEFAULT_FROM_EMAIL, [application.email])
    ...
    # 3. إرسال SMS (قيد التنفيذ - Placeholder للربط المستقبلي)
    if status == 'accepted':
...
def _send_sms_placeholder(phone, message):
    ...
    # تنظيف رقم الجوال (يجب أن يبدأ بـ 966 أو كود الدولة)
    ...
    try:
        # ملاحظة: تم استخدام Unifonic Rest API كمثال افتراضي للسوق السعودي
