    ...
    return {
        ...
        "edx_org_link": {
            "url": configuration_helpers.get_value(
                'PLATFORM_ORG_LINK_URL',
                "https://www.edx.org/?utm_medium=affiliate_partner"
                "&utm_source=opensource-partner"
                "&utm_content=open-edx-partner-footer-link"
                "&utm_campaign=open-edx-footer",
            ),
            "text": configuration_helpers.get_value(
                'PLATFORM_ORG_LINK_TEXT',
...
def _footer_copyright():
    ...
    custom_copyright = configuration_helpers.get_value('PLATFORM_COPYRIGHT', None)
    if custom_copyright:
        return custom_copyright.format(org_name=org_name)
    return _("© {org_name}. All rights reserved.").format(org_name=org_name)
